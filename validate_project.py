...
for needle in ["appVersion:'24.58.1'","release:'R58'","dataSchema:3"]:
...
try:
    ...
    import hashlib
    if rel.get('appVersion')!='24.58.1' or rel.get('release')!='R58': FAIL.append('release-manifest version mismatch')
...
if "script-src 'self' 'unsafe-inline'" in idx: FAIL.append('main CSP must not allow unsafe-inline scripts')
if re.search(r'<script(?![^>]*\bsrc=)[^>]*>',idx,re.I): FAIL.append('index.html contains inline executable script')
