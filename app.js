...
let PROPHET_STORIES=null,QURAN_STORIES=null,storyDetailHistory=false,storyDetailMode='',storyDetailId='',storyDetailReturnFocus=null,storyQueries={prophets:'',qstories:''};
...
let knowledgeSubpageHistory=false,knowledgeCategoryHistory=false,knowledgeRenderSeq=0;
...
function knowledgeLoading(mode){
  const host=document.getElementById('learn-'+mode);if(!host)return;
  if(mode==='riyad'){const count=document.getElementById('rs-result-count');if(count)count.textContent='جارٍ تحميل المحتوى…';return}
  host.innerHTML='<div class="nafs-empty" role="status" aria-live="polite">جارٍ تحميل المحتوى…</div>';
}
function knowledgeLoadError(mode,error){
  const host=document.getElementById('learn-'+mode);if(!host)return;
  host.innerHTML=`<div class="nafs-empty" role="alert"><b>تعذر إظهار المحتوى الآن.</b><br><span>حاول فتح الباب مرة أخرى. ${laterEsc(error?.message||'')}</span><br><button type="button" data-knowledge-retry>إعادة المحاولة</button></div>`;
  host.querySelector('[data-knowledge-retry]')?.addEventListener('click',()=>renderKnowledge());
}
...
async function enterKnowledgeSubpage(mode,parent=learnCategory,{pushHistory=true,query=''}={}){if(!mode)return;closeHadithDetail();if(parent&&KNOWLEDGE_CATS[parent])learnCategory=parent;learnMode=mode;learnQuery=query||'';learnGroup='all';if(['prophets','qstories'].includes(mode))storyQueries[mode]=query||'';nawawiQuery=mode==='nawawi'?(query||''):'';if(pushHistory&&!knowledgeSubpageHistory){try{history.pushState({...(history.state||{}),tadaruqKnowledge:true,knowledgeMode:mode,knowledgeParent:learnCategory||''},'',location.href);knowledgeSubpageHistory=true}catch{}}const shell=document.getElementById('v-sunnah');if(shell)shell.scrollTo({top:0,behavior:'auto'});await renderKnowledge()}
...
function storyModeLabel(mode){return mode==='prophets'?'قصص الأنبياء':'قصص القرآن'}
function storyRefLabel(r){
  const su=Q?.suras?.[(+r.sura||1)-1],name=su?.name||`سورة ${AR(r.sura)}`,range=(+r.to||+r.from)>(+r.from)?`${AR(r.from)}–${AR(r.to)}`:AR(r.from);
  return `${name} · ${range}`;
}
function storyRefUrl(r){return `https://quran.com/${encodeURIComponent(r.sura)}/${encodeURIComponent(r.from)}`}
function storySearchText(x){return searchNorm([x.title,x.subtitle,x.summary,...(x.keywords||[]),...(x.timeline||[]).flatMap(y=>[y.title,y.text]),...(x.lessons||[]),...(x.cautions||[])].join(' '))}
async function renderStoryLibrary(mode){
  const host=document.getElementById('learn-'+mode);if(!host)return;
  const data=await loadStoryLibrary(mode),m=data?.meta||{},all=data?.items||[],q=searchNorm(storyQueries[mode]||'');
  const items=all.filter(x=>!q||storySearchText(x).includes(q));
  host.innerHTML=`<section class="learn-source-first story-source-first"><small>المصدر والمنهج قبل المحتوى</small><h3>${laterEsc(m.title||storyModeLabel(mode))}</h3><p>${laterEsc(m.subtitle||'')}</p><div class="source-stack">${sourceStack([m.primarySource,m.tafsirSource].filter(Boolean))}</div><div class="learn-method-panel"><b>منهج العرض:</b> ${laterEsc(m.methodology||'')}</div>${m.note?`<span class="learn-note">${laterEsc(m.note)}</span>`:''}</section><div class="story-tools"><input class="q-search" id="story-search-${mode}" type="search" value="${laterEsc(storyQueries[mode]||'')}" placeholder="ابحث داخل ${laterEsc(storyModeLabel(mode))}…" aria-label="بحث داخل ${laterEsc(storyModeLabel(mode))}"><div class="search-count">${AR(items.length)} من ${AR(all.length)} قصة</div></div><div class="story-grid">${items.map((x,i)=>`<button type="button" class="story-card" data-story-id="${laterEsc(x.id)}"><span class="story-no">${AR(i+1)}</span><span class="story-card-copy"><small>${laterEsc(storyModeLabel(mode))}</small><b>${laterEsc(x.title)}</b><span>${laterEsc(x.subtitle||x.summary||'')}</span><em>${AR((x.refs||[]).length)} موضع قرآني · ${AR((x.timeline||[]).length)} محطة</em></span></button>`).join('')}</div>${items.length?'':'<div class="structured-empty">لا توجد نتيجة مطابقة. جرّب كلمة أقصر أو اسم شخصية/موضع.</div>'}`;
  host.querySelector(`#story-search-${mode}`)?.addEventListener('input',e=>{storyQueries[mode]=e.target.value||'';renderStoryLibrary(mode)});
  host.querySelector('.story-grid')?.addEventListener('click',e=>{const b=e.target.closest('[data-story-id]');if(b)openStoryDetail(mode,b.dataset.storyId)});
}
async function openStoryDetail(mode,id,fromPop=false){
  const data=await loadStoryLibrary(mode),x=(data?.items||[]).find(v=>v.id===id),detail=document.getElementById('story-detail');if(!x||!detail)return;
  await loadQuran();storyDetailReturnFocus=document.activeElement;storyDetailMode=mode;storyDetailId=id;
  document.getElementById('story-detail-kicker').textContent=storyModeLabel(mode);
  const m=data.meta||{},refs=x.refs||[],firstRef=refs[0];
  document.getElementById('story-detail-content').innerHTML=`<header class="story-detail-hero"><small>${laterEsc(x.subtitle||storyModeLabel(mode))}</small><h2 id="story-detail-title">${laterEsc(x.title)}</h2><p>${laterEsc(x.summary||'')}</p></header><section class="story-detail-block"><div class="story-section-kicker">تسلسل القصة</div><h3>المحطات الأساسية</h3>${(x.timeline||[]).map((p,i)=>`<div class="story-timeline-item"><div class="story-section-kicker">المحطة ${AR(i+1)}</div><h3>${laterEsc(p.title)}</h3><p>${laterEsc(p.text)}</p></div>`).join('')}</section><section class="story-detail-block story-lessons"><div class="story-section-kicker">ما الذي نتعلمه؟</div><h3>دروس من القصة</h3><ul>${(x.lessons||[]).map(v=>`<li>${laterEsc(v)}</li>`).join('')}</ul></section>${x.cautions?.length?`<section class="story-detail-block story-cautions"><div class="story-section-kicker">ضبط الرواية</div><h3>ما الذي لا نجزم به؟</h3><ul>${x.cautions.map(v=>`<li>${laterEsc(v)}</li>`).join('')}</ul></section>`:''}<section class="story-detail-block"><div class="story-section-kicker">المصدر الأول</div><h3>مواضع القصة في القرآن</h3><div class="story-refs">${refs.map(r=>`<a href="${storyRefUrl(r)}" target="_blank" rel="noopener"><b>${laterEsc(storyRefLabel(r))}</b><small>فتح الموضع في القرآن</small></a>`).join('')}</div>${firstRef?`<button type="button" class="story-tafsir-btn" data-story-tafsir="${firstRef.sura}">افتح التفسير الميسر لهذه السورة</button>`:''}</section><section class="story-detail-block story-method"><div class="story-section-kicker">المنهج والمراجع</div><h3>كيف كُتبت هذه المادة؟</h3><p>${laterEsc(m.methodology||'')}</p><div class="source-stack">${sourceStack([m.primarySource,m.tafsirSource].filter(Boolean))}</div>${m.note?`<p>${laterEsc(m.note)}</p>`:''}</section>`;
  detail.classList.remove('hide');detail.setAttribute('aria-hidden','false');document.body.classList.add('story-detail-open');detail.scrollTop=0;
  detail.querySelector('[data-story-tafsir]')?.addEventListener('click',e=>openMuyassarSheet(+e.currentTarget.dataset.storyTafsir||1));
  if(!fromPop&&!storyDetailHistory){try{history.pushState({...(history.state||{}),tadaruqStory:true,storyMode:mode,storyId:id},'',location.href);storyDetailHistory=true}catch{}}
  setTimeout(()=>document.getElementById('story-detail-back')?.focus({preventScroll:true}),30);
}
function closeStoryDetail(fromPop=false){
  const detail=document.getElementById('story-detail');if(!detail||detail.classList.contains('hide'))return;
  if(!fromPop&&storyDetailHistory){try{history.back();return}catch{}}
  detail.classList.add('hide');detail.setAttribute('aria-hidden','true');document.body.classList.remove('story-detail-open');storyDetailHistory=false;
  if(storyDetailReturnFocus&&typeof storyDetailReturnFocus.focus==='function')storyDetailReturnFocus.focus({preventScroll:true});
  storyDetailReturnFocus=null;storyDetailMode='';storyDetailId='';
}
document.getElementById('story-detail-back')?.addEventListener('click',()=>closeStoryDetail());
document.addEventListener('keydown',e=>{if(e.key==='Escape'&&!document.getElementById('story-detail')?.classList.contains('hide'))closeStoryDetail()});
window.addEventListener('popstate',e=>{if(storyDetailHistory&&!e.state?.tadaruqStory&&!document.getElementById('story-detail')?.classList.contains('hide'))closeStoryDetail(true)});

async function renderKnowledge(){
  const seq=++knowledgeRenderSeq,mode=learnMode;
  const modes=['agreed','riyad','nawawi','seerah','companions','essentials','fiqh','busola','muyassar','prophets','qstories','usultafsir','usulfiqh','fuqaha','history','aqeedah','fiqhlife','tajweed','hadithsciences','akhlaq','adab','digital','qawaid'];
  modes.forEach(x=>document.getElementById('learn-'+x)?.classList.toggle('hide',x!==mode));
  if(mode==='home'){openKnowledgeHome();return}
  if(mode==='category'){openKnowledgeCategory(learnCategory);return}
  document.getElementById('knowledge-home')?.classList.add('hide');
  setKnowledgeSubpageShell(true);
  const found=Object.entries(KNOWLEDGE_CATS).find(([,c])=>c.items.some(([id])=>id===mode));
  if(found){learnCategory=found[0];const c=found[1],head=document.getElementById('learn-category-head');head?.classList.remove('hide');document.getElementById('learn-category-title').textContent=c.items.find(([id])=>id===mode)?.[1]||c.title;document.getElementById('learn-category-kicker').textContent=c.title;document.getElementById('learn-category-back').textContent=`→ رجوع إلى ${c.title}`;document.getElementById('learn-seg')?.classList.add('hide')}
  else{const head=document.getElementById('learn-category-head');head?.classList.remove('hide');document.getElementById('learn-category-title').textContent=mode==='essentials'?'ما لا يسع المسلم جهله':'العلم';document.getElementById('learn-category-kicker').textContent='صفحة مستقلة';document.getElementById('learn-category-back').textContent='→ رجوع إلى خريطة العلم';document.getElementById('learn-seg')?.classList.add('hide')}
  knowledgeLoading(mode);
  try{
    if(['prophets','qstories'].includes(mode)){await renderStoryLibrary(mode);return}
    if(['muyassar','usultafsir','usulfiqh','fuqaha','history'].includes(mode)){await renderSimpleKnowledge(mode);return}
    if(['aqeedah','fiqhlife','tajweed','hadithsciences','akhlaq','adab','digital','qawaid'].includes(mode)){await renderStructuredKnowledge(mode);return}
    if(mode==='agreed'){await renderAgreed();return}
    if(mode==='riyad'){await renderSunnah();return}
    if(mode==='nawawi'){await renderNawawi();return}
    if(mode==='seerah'){await renderSeerah();return}
    if(mode==='companions'){await renderCompanions();return}
    if(mode==='busola'){await renderBusola();return}
    await loadLearn();
    if(seq!==knowledgeRenderSeq||learnMode!==mode)return;
    const items=mode==='essentials'?LEARN.essentials:LEARN.fiqh,fiqh=mode==='fiqh',F=filteredLearn(items);const intro=fiqh?`<section class="learn-source-first"><small>المصدر والمنهج قبل المحتوى</small><h3>الفقه الميسر — ابدأ من هنا</h3><p>${laterEsc(LEARN.meta.fiqhBook)}</p><div class="source-stack">${sourceStack([{label:LEARN.meta.fiqhBook,url:LEARN.meta.fiqhUrl}])}</div><div class="learn-method-panel">هذا هو المدخل الأساسي للعامة. بعده استخدم «فقه حياتي» للوصول السريع للمسألة بحسب حالك.</div></section>`:`<section class="learn-source-first"><small>المصدر والمنهج قبل المحتوى</small><h3>ما لا يسع المسلم جهله</h3><p>${laterEsc(LEARN.meta.essentialsBook)}</p><div class="source-stack">${sourceStack([{label:LEARN.meta.essentialsBook,url:LEARN.meta.essentialsUrl}])}</div>${LEARN.meta.essentialsNote?`<div class="learn-method-panel">${laterEsc(LEARN.meta.essentialsNote)}</div>`:''}</section>`;const host=document.getElementById('learn-'+mode);if(!host)return;host.innerHTML=`${intro}${learnFilters(items)}<div class="search-count">${AR(F.length)} من ${AR(items.length)} بابًا</div>${learnCards(F,fiqh)}<div class="learn-disclaimer">${LEARN.meta.note}</div>`;host.querySelector('#learn-search').oninput=e=>{learnQuery=e.target.value;renderKnowledge()};host.querySelector('.learn-groups').onclick=e=>{const b=e.target.closest('button[data-lg]');if(!b)return;learnGroup=b.dataset.lg;renderKnowledge()};
  }catch(error){
    if(seq===knowledgeRenderSeq&&learnMode===mode)knowledgeLoadError(mode,error);
  }
}
