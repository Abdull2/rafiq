...
function structuredSourceIntro(data){const m=data?.meta||{},sources=m.sources||[];return `<section class="learn-source-first"><small>المصدر والمنهج قبل المحتوى</small><h3>${laterEsc(m.title||'')}</h3><p>${laterEsc(m.subtitle||'')}</p><div class="source-stack">${sourceStack(sources,m.book||m.source||'',m.url||'')}</div>${m.method?`<div class="learn-method-panel"><b>منهج العرض:</b> ${laterEsc(m.method)}</div>`:''}${m.note?`<p>${laterEsc(m.note)}</p>`:''}</section>`}
...
function zakatCashEstimator(){return `<section class="zakat-estimator"><small>أداة حسابية محدودة — ليست فتوى</small><h3>تقدير ٢٫٥٪ للنقود والمدخرات فقط</h3><p>استخدمها فقط بعد أن تتحقق بنفسك أن المال من النوع الذي يزكى بهذه النسبة وأن شروط النصاب والحول متحققة. لا تستعملها للزروع أو الأنعام أو المسائل الاستثمارية والديون المركبة.</p><label>المبلغ الذي قررت أنه داخل في وعاء الزكاة<input id="zakat-cash-input" inputmode="decimal" type="number" min="0" step="0.01" placeholder="مثال: 100000"></label><button type="button" id="zakat-cash-calc">احسب ٢٫٥٪</button><output id="zakat-cash-output" aria-live="polite"></output><div class="source-stack">${sourceStack([{label:'الفقه الميسر — كتاب الزكاة',url:'https://islamhouse.com/ar/books/203277'},{label:'الموسوعة الفقهية — الدرر السنية',url:'https://dorar.net/feqhia'}])}</div></section>`}
function structuredKnowledgeCards(items,kind='علم'){if(!items.length)return '<div class="structured-empty">لا توجد نتائج مطابقة. جرّب كلمة أقصر أو اختر «كل الأبواب».</div>';return items.map((x,i)=>{const st=x.status||'',tone=structuredStatusTone(st);return `<article class="learn-module-card">${st?`<div class="learn-status" data-tone="${tone}">${laterEsc(st)}</div>`:''}<h3>${laterEsc(x.title||x.name)}</h3><div class="lead">${laterEsc(x.lead||'')}</div>${x.points?.length?`<ul>${x.points.map(p=>`<li>${laterEsc(p)}</li>`).join('')}</ul>`:''}<div class="source-stack">${sourceStack(x.sources||[])}</div>${laterRegister(`${kind}:${x.id}`,{kind,title:x.title||x.name,text:x.lead||'',source:x.sources?.[0]?.label||'',tab:'sunnah'})}</article>`}).join('')}
async function renderStructuredKnowledge(mode){const cfg={aqeedah:[loadAqeedah,'عقيدة'],fiqhlife:[loadFiqhLife,'فقه حياتي'],tajweed:[loadTajweed,'تجويد'],hadithsciences:[loadHadithSciences,'علوم حديث'],akhlaq:[loadAkhlaq,'أخلاق'],adab:[loadAdab,'أدب شرعي'],digital:[loadDigitalLife,'حياة رقمية'],qawaid:[loadQawaid,'قاعدة فقهية']}[mode];if(!cfg)return;const data=await cfg[0](),items=data?.items||[],host=document.getElementById('learn-'+mode);if(!host)return;const F=filteredLearn(items),quick=mode==='fiqhlife'?fiqhLifeQuickBar():'',zakat=mode==='fiqhlife'&&learnGroup==='الزكاة'?zakatCashEstimator():'';host.innerHTML=structuredSourceIntro(data)+quick+learnFilters(items)+zakat+`<div class="curriculum-count">${AR(F.length)} من ${AR(items.length)} موضوعًا</div>`+structuredKnowledgeCards(F,cfg[1]);host.querySelector('#learn-search').oninput=e=>{learnQuery=e.target.value;renderStructuredKnowledge(mode)};host.querySelector('.learn-groups').onclick=e=>{const b=e.target.closest('button[data-lg]');if(!b)return;learnGroup=b.dataset.lg;renderStructuredKnowledge(mode)};host.querySelector('.fiqh-quick')?.addEventListener('click',e=>{const b=e.target.closest('[data-fiqh-group]');if(!b)return;learnGroup=b.dataset.fiqhGroup;learnQuery='';renderStructuredKnowledge(mode)});host.querySelector('#zakat-cash-calc')?.addEventListener('click',()=>{const inp=host.querySelector('#zakat-cash-input'),out=host.querySelector('#zakat-cash-output'),n=Number(inp?.value);if(!Number.isFinite(n)||n<0){out.textContent='أدخل مبلغًا صحيحًا غير سالب.';return}out.textContent=`التقدير الحسابي: ${(n*0.025).toLocaleString('ar-EG',{maximumFractionDigits:2})} — راجع شروط الزكاة في البطاقات أدناه.`})}
...
function seerahCards(items){return items.map((x,i)=>{const plus=laterRegister(`seerah:${x.id}`,{kind:'سيرة',title:x.title,text:x.lead,source:x.sourceLabel,tab:'sunnah'});return `<details class="learn-card seerah-card"><summary><span class="ln">${AR(i+1)}</span><span class="learn-summary-copy"><span class="lt">${laterEsc(x.title)}</span><span class="learn-summary-hint">خلاصة · تفاصيل · تربويات</span></span>${plus}</summary><div class="learn-body"><div class="learn-depth-head soft">الخلاصة السريعة</div><p class="learn-lead">${laterEsc(x.lead)}</p>${x.bullets?.length?`<ul>${x.bullets.map(b=>`<li>${laterEsc(b)}</li>`).join('')}</ul>`:''}${mediumSections(x.detailSections||[])}${seerahReflections(x)}<div class="source-stack">${sourceStack(x.sources||[],x.sourceLabel,x.sourceUrl)}</div></div></details>`}).join('')}
...
async function renderKnowledge(){
  await loadLearn();const modes=['agreed','riyad','nawawi','seerah','companions','essentials','fiqh','busola','muyassar','usultafsir','usulfiqh','fuqaha','history','aqeedah','fiqhlife','tajweed','hadithsciences','akhlaq','adab','qawaid'];modes.forEach(x=>document.getElementById('learn-'+x)?.classList.toggle('hide',x!==learnMode));
  ...
  const items=learnMode==='essentials'?LEARN.essentials:LEARN.fiqh,fiqh=learnMode==='fiqh',F=filteredLearn(items);const intro=fiqh?`<b>الفقه الميسر — ابدأ من هنا</b><br>${LEARN.meta.fiqhBook}<br><a href="https://islamhouse.com/ar/books/203277" target="_blank" rel="noopener">فتح صفحة الكتاب ومصدره ↗</a><br>هذا هو المدخل الأساسي للعامة. بعده استخدم «فقه حياتي» للوصول السريع للمسألة بحسب حالك.`:`<b>ما لا يسع المسلم جهله</b><br>${LEARN.meta.essentialsBook}<br><a href="${LEARN.meta.essentialsUrl}" target="_blank" rel="noopener">فتح المرجع العام ↗</a><br>${LEARN.meta.essentialsNote||''}`;const host=document.getElementById('learn-'+learnMode);host.innerHTML=`<div class="learn-intro">${intro}</div>${learnFilters(items)}<div class="search-count">${AR(F.length)} من ${AR(items.length)} بابًا</div>${learnCards(F,fiqh)}<div class="learn-disclaimer">${LEARN.meta.note}</div>`;host.querySelector('#learn-search').oninput=e=>{learnQuery=e.target.value;renderKnowledge()};host.querySelector('.learn-groups').onclick=e=>{const b=e.target.closest('button[data-lg]');if(!b)return;learnGroup=b.dataset.lg;renderKnowledge()};
}
...
async function hManazil(){
  ...
  host.innerHTML=`<button class="back" id="manazil-back">→ خريطة التزكية</button><div class="manazil-hero"><small>قراءة تربوية موثقة</small><h2>${laterEsc(m.title||'منازل السائرين')}</h2><p>${laterEsc(m.subtitle||'')}</p><div class="manazil-note">${laterEsc(m.note||'')}</div><a href="${laterEsc(m.url||'')}" target="_blank" rel="noopener">${laterEsc(m.book||'فتح المرجع')} ↗</a></div><div class="manazil-grid">${items.map((x,i)=>`${laterRegister(`manazil:${x.id}`,{kind:'تزكية',title:x.title,text:x.lead,source:m.book||'مدارج السالكين',tab:'qalb'})}<article class="manzil-card"><div class="manzil-num">${AR(i+1)}</div><h3>${laterEsc(x.title)}</h3><p class="manzil-lead">${laterEsc(x.lead||'')}</p>${x.points?.length?`<ul>${x.points.map(p=>`<li>${laterEsc(p)}</li>`).join('')}</ul>`:''}<a href="${laterEsc(x.sourceUrl||m.url||'')}" target="_blank" rel="noopener">موضع المنزلة في المرجع ↗</a></article>`).join('')}</div>`;
...
async function hObstacles(){
  ...
  document.getElementById('v-qalb').innerHTML=`<div class="h-tabs">${hPathTab(false)}<button data-k="problems">أمراض القلوب</button><button data-k="works">أعمال القلوب</button><button data-k="obstacles" aria-current="true">العقبات</button><button data-k="deeds">بنك الأعمال</button><button data-k="nafs">فقه النفس</button>${hBenefitTab(false)}${issueTabButton(false)}</div><div class="nafs-hero"><h2>${genderText('مشكلتك مختلفة… والدين ثابت.','مشكلتك مختلفة… والدين ثابت.')}</h2><p>العقبات هنا للحياة الواقعية: أسرة، دراسة، عمل، علاقات، هاتف، فتور، قلق وأسئلة. ${genderText('اختر ما يشبه موقفك','اختاري ما يشبه موقفكِ')}؛ وكل جواب وخطوة تحته مصدره.</p></div><input id="heart-search" class="q-search" type="search" value="${laterEsc(hQuery)}" placeholder="ابحث: أسرة، علاقة، دراسة، قلق، هاتف…"><div class="search-count" id="heart-result-count"></div><div id="heart-list"></div>`;
  const render=()=>{
