...
/* ================= العلم: رياض الصالحين + الأربعون النووية + السيرة المختصرة + الأساسيات + الفقه ================= */
let LEARN=null, SEERAH=null, learnMode='riyad', learnQuery='', learnGroup='all';
...
function seerahCards(items){return items.map((x,i)=>{const plus=laterRegister(`seerah:${x.id}`,{kind:'سيرة',title:x.title,text:x.lead,source:x.sourceLabel,tab:'sunnah'});return `<details class="learn-card seerah-card"><summary><span class="ln">${AR(i+1)}</span><span class="lt">${laterEsc(x.title)}</span>${plus}</summary><div class="learn-body"><p class="learn-lead">${laterEsc(x.lead)}</p>${x.bullets?.length?`<ul>${x.bullets.map(b=>`<li>${laterEsc(b)}</li>`).join('')}</ul>`:''}<div class="learn-source">المرجع: <a href="${laterEsc(x.sourceUrl)}" target="_blank" rel="noopener">${laterEsc(x.sourceLabel)}</a></div></div></details>`}).join('')}
async function renderSeerah(){await loadSeerah();const host=document.getElementById('learn-seerah'),items=SEERAH.items||[],q=searchNorm(learnQuery),groups=['all',...new Set(items.map(x=>x.group).filter(Boolean))],F=items.filter(x=>(learnGroup==='all'||x.group===learnGroup)&&(!q||searchNorm(deepSearchText(x)).includes(q))),m=SEERAH.meta||{};host.innerHTML=`<div class="learn-intro"><b>${laterEsc(m.title||'مختصر السيرة النبوية')}</b><br>المرجع المعتمد لهذا المختصر: <a href="${laterEsc(m.url)}" target="_blank" rel="noopener">${laterEsc(m.book||'رسول الإسلام محمد ﷺ')}</a><br><span style="font-size:12px;color:var(--soft)">${laterEsc(m.author||'')}</span><br><span style="font-size:12px">${laterEsc(m.note||'')}</span></div><div class="learn-tools"><input id="seerah-search" class="q-search" type="search" value="${laterEsc(learnQuery)}" placeholder="ابحث في محطات السيرة…"><div class="learn-groups">${groups.map(x=>`<button data-sg="${laterEsc(x)}" class="${learnGroup===x?'on':''}">${x==='all'?'كل المحطات':laterEsc(x)}</button>`).join('')}</div></div><div class="search-count">${AR(F.length)} من ${AR(items.length)} محطة</div>${seerahCards(F)}<div class="learn-intro" style="margin-top:12px"><b>للتوسع في السيرة</b><br><a href="${laterEsc(m.expansionUrl)}" target="_blank" rel="noopener">${laterEsc(m.expansionTitle||'فتح مرجع التوسع')}</a></div>`;host.querySelector('#seerah-search').oninput=e=>{learnQuery=e.target.value;renderSeerah()};host.querySelector('.learn-groups').onclick=e=>{const b=e.target.closest('button[data-sg]');if(!b)return;learnGroup=b.dataset.sg;renderSeerah()}}
...
async function renderKnowledge(){
  await loadLearn();document.querySelectorAll('#learn-seg button').forEach(b=>b.setAttribute('aria-current',b.dataset.learn===learnMode));['riyad','nawawi','seerah','essentials','fiqh'].forEach(x=>document.getElementById('learn-'+x)?.classList.toggle('hide',x!==learnMode));
  if(learnMode==='riyad'){renderSunnah();return}if(learnMode==='nawawi'){renderNawawi();return}if(learnMode==='seerah'){renderSeerah();return}
