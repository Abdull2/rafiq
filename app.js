...
function openMuyassarSheet(){
  ...
  sh.classList.remove('hide');sh.setAttribute('aria-hidden','false');document.body.classList.add('muyassar-open');document.getElementById('muyassar-full-link')?.focus();
}
...
let LEARN=null, SEERAH=null, COMPANIONS=null, BUSOLA=null, AGREED=null, USUL_TAFSIR=null,USUL_FIQH=null,FUQAHA=null,HISTORY=null,busolaMode='maqasid', busolaOpen=null, busolaProgress={}, learnMode='home', learnCategory='', learnQuery='', learnGroup='all';
...
function agreedCards(items){return items.map((x,i)=>`${laterRegister(`agreed:${x.id}`,{kind:'حديث متفق عليه',title:x.title,text:x.text,source:'صحيح البخاري وصحيح مسلم',tab:'sunnah'})}<article class="agreed-card"><div class="agreed-top"><span>حديث ${AR(i+1)}</span><span>البخاري ${laterEsc(x.bukhari)} · مسلم ${laterEsc(x.muslim)}</span></div><h3>${laterEsc(x.title)}</h3><div class="agreed-text">«${laterEsc(x.text)}»</div><div class="agreed-narrator">${laterEsc(x.narrator||'')}</div><div class="agreed-explain"><b>شرح مبسط</b><p>${laterEsc(x.meaning||'')}</p></div><div class="agreed-practice"><b>تطبيق</b><p>${laterEsc(x.practice||'')}</p></div><a class="agreed-source" href="${laterEsc(x.sourceUrl||'')}" target="_blank" rel="noopener">الحديث وشرحه في الدرر السنية ↗</a></article>`).join('')}
async function renderAgreed(){await loadAgreed();const host=document.getElementById('learn-agreed');if(!host)return;const m=AGREED.meta||{},items=AGREED.items||[],q=searchNorm(learnQuery),F=items.filter(x=>!q||searchNorm(deepSearchText(x)).includes(q));host.innerHTML=`<div class="learn-intro agreed-intro"><b>${laterEsc(m.title||'أحاديث متفق عليها')}</b><br>${laterEsc(m.subtitle||'')}<div class="learn-ref-row"><a href="${laterEsc(m.url||'https://dorar.net/hadith/search')}" target="_blank" rel="noopener">${laterEsc(m.source||'الموسوعة الحديثية — الدرر السنية')} ↗</a></div><span class="learn-note">${laterEsc(m.note||'')}</span></div><input id="agreed-search" class="q-search" type="search" value="${laterEsc(learnQuery)}" placeholder="ابحث: النية، الرحمة، الغضب، الأمانة…" aria-label="بحث في الأحاديث المتفق عليها"><div class="search-count">${AR(F.length)} من ${AR(items.length)} حديثًا</div><div class="agreed-grid">${agreedCards(F)}</div>`;host.querySelector('#agreed-search').oninput=e=>{learnQuery=e.target.value;renderAgreed()}}
...
async function renderKnowledge(){
  await loadLearn();const modes=['agreed','riyad','nawawi','seerah','companions','essentials','fiqh','busola','muyassar','usultafsir','usulfiqh','fuqaha','history'];modes.forEach(x=>document.getElementById('learn-'+x)?.classList.toggle('hide',x!==learnMode));
...
function switchTab(t){
  ...
  const navFor={read:'quran',zikrhub:'azkar',dua:'azkar',tasbih:'azkar',asma:'azkar'}[t]||t;
