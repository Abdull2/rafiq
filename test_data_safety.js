...
(async()=>{
  ...
  localStorage.setItem('qalb-levels-v1',JSON.stringify({ikhlas:{done:[1,2],open:3,reviews:[1724100000000]}}));
  ...
  const qlv=JSON.parse(localStorage.getItem('qalb-levels-v1'));
  if(!qlv?.ikhlas||qlv.ikhlas.open!==3||qlv.ikhlas.done.length!==2)throw new Error('qalb levels backup/restore failed');
  ...
  console.log('DATA_SAFETY_TEST_OK',JSON.stringify({version:audit.dataVersion,records:audit.recordCount,saved:audit.savedCount,days:audit.dayCount,qalbLevels:Object.keys(JSON.parse(localStorage.getItem('qalb-levels-v1')||'{}')).length}));
